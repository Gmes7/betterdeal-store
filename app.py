...
def send_order_notification(order):
    ...
    try:
        # This is a basic example - configure with your SMTP settings
        msg = MIMEMultipart()
        msg['From'] = ADMIN_EMAIL
        msg['To'] = ADMIN_EMAIL
        msg['Subject'] = f"New Order #{order['id']} - BetterDeal"
        ...
        for item in order['items']:
            body += f"- {item['name']} x {item['quantity']} = ${item['total']}\n"
        ...
        # server.starttls()
        # server.login(ADMIN_EMAIL, 'your-email-password')
        # server.send_message(msg)
        # server.quit()

        print(f"📧 Order notification would be sent for order #{order['id']}")
...
def get_financial_reports():
    ...
    profit_growth = ((current_month_profit - last_month_profit) / abs(
        last_month_profit) * 100) if last_month_profit != 0 else 0
    ...
    return {
        ...
        'expenses': {
            ...
            'growth': ((
                                   current_month_expense_total - last_month_expense_total) / last_month_expense_total * 100) if last_month_expense_total > 0 else 0
        },
        'profit': {
...
# Add these new routes after existing admin routes
